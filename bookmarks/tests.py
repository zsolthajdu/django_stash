...
class ViewTestCase(TestCase):
	...
	def setUp(self):
		...
		oldDate = "2018-08-28T13:07:10-07:00"
		self.bookmark_data = { 'url':'https://cnn.com',
				'description':'Global news network', 'owner': self.alice.id, 'tags': ['tv,news'], 
				'public':'true', 'created':oldDate }
		self.response = self.clientA.post( reverse('create'), self.bookmark_data, format="json")
	...
	def test_date_search_results_year_month_day(self):
		"""Test Date-search results."""
		new_client = APIClient()
		today = datetime.date.today()
		resp = new_client.get( reverse('find-year_month_day', kwargs={'year': today.year,'month':today.month,'day':today.day}), format='json', follow=True)
		# Only one should have today's date
		self.assertEqual( resp.data['count'], 1 )

	def test_date_search_results_year_month(self):
		"""Test Date-search results."""
		new_client = APIClient()
		resp = new_client.get( reverse('find-year_month', kwargs={'year': 2018,'month':8 } ), format='json', follow=True)
		# Only one should have today's date
		self.assertEqual( resp.data['count'], 1 )
